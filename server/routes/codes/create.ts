...
export default defineEventHandler(async (event)=>{
    ...
    const { tax_code, description, current_rate, previous_rate, basis, effective_date,sales_ledger_code, ap_ledger_code } = await readBody(event);
        try {
        const createTaxCode = await prisma.taxCode.create({
            data: {
                tax_code: tax_code,
                description: description,
                current_rate: parseFloat(current_rate),
                previous_rate: parseFloat(previous_rate),
                basis: basis,
                effective_date: new Date(effective_date),
                sales_ledger_code: sales_ledger_code,
                ap_ledger_code: ap_ledger_code,
            }
        });
        response['registred'] = createTaxCode
    ...
    //                                 Dear ${name} ${surname}<br><br>Your Oacey HCM Registration is complete, please await approval from the systems adminstrator<br><br>From<br><br>Oacey Systems
    //                                 </td>
    //                             </tr>
    //                         </tbody></table>
    //                     </td>
